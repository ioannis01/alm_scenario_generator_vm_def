            ...
            <div class="logo">
                <div class="logo-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" style="width: 40px; height: 40px;">
                        <rect x="32.5" y="27" width="22.5" height="11.5" fill="#3FDB60"/>
                        <rect x="56" y="38.5" width="11.5" height="11.5" fill="#3FDB60"/>
                        <rect x="32.5" y="50" width="23.5" height="11.5" fill="#3FDB60"/>
                        <rect x="56" y="61.5" width="11.5" height="11.5" fill="#3FDB60"/>
                    </svg>
                </div>
                <div class="logo-text">
...
Generate 3 stress scenarios for interest rate risk, credit spreads, and FX rates with severe but plausible shocks based on historical crisis events."
                    ...
                    <!-- Inline Results Display (appears after generation) -->
                    <div id="inlineResults" class="hidden" style="margin-top: 2rem;">
        ...
        <div class="tab-content" id="analysis-content">
            <div class="grid" style="grid-template-columns: 1fr;">
                <div class="card" style="height: calc(100vh - 200px); display: flex; flex-direction: column;">
                    <div class="card-header">
                        <h2 class="card-title">🤖 AI-Powered Analysis Assistant</h2>
                        <p style="color: var(--muted); font-size: 0.875rem; margin-top: 0.5rem;">
                            Ask questions about your data, request analysis, or get explanations. The AI uses your loaded data, knowledge, and selected prompt.
                        </p>
                    </div>
                    
                    <!-- Chat History -->
                    <div id="chatHistory" style="flex: 1; overflow-y: auto; padding: 1rem; background: var(--card-dark); border-radius: 8px; margin-bottom: 1rem;">
                        <div class="empty-state" id="chatEmptyState">
                            <div class="empty-icon">💬</div>
                            <p>Start a conversation with the AI</p>
                            <p style="font-size: 0.875rem; color: var(--muted);">
                                Example questions:<br>
                                • "What types of risk factors are loaded?"<br>
                                • "Analyze the duration structure of my contracts"<br>
                                • "Explain my counterparty exposure by country"<br>
                    ...
                    <div style="border-top: 1px solid var(--border); padding-top: 1rem;">
                        ...
                        <div style="margin-bottom: 1rem; display: flex; gap: 1rem; align-items: center;">
                            <label style="color: var(--muted); font-size: 0.875rem; white-space: nowrap;">
                                💡 LLM Prompt:
                            </label>
                            <select id="analysisPromptSelect" class="form-control" style="flex: 1;">
                                <option value="">Loading prompts...</option>
                            </select>
                            <div style="color: var(--muted); font-size: 0.75rem;" id="analysisDataStatus">
                                Data: Not loaded
                            </div>
                        </div>
                        ...
                        <div style="display: flex; gap: 0.5rem;">
                            <textarea
                                id="analysisMessage"
                                placeholder="Ask me anything about your data, request analysis, or get explanations..."
                                rows="3"
                                class="form-control"
                                style="flex: 1; resize: vertical; min-height: 60px;"
                                onkeydown="if(event.ctrlKey && event.key==='Enter') sendAnalysisMessage()"
                            ></textarea>
                            <button
                                id="sendAnalysisBtn"
...
Counterparties: ${data.counterparties_count.toLocaleString()}`;
        ...
        function handleGenerate(event) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (data.reasoning) {
                        addProcessStep('AI reasoning generated - view in results below', 'success');
                    }
                    ...
                    // Pass reasoning and context to display function
                    displayResults(data.scenarios, data.report, data.csv_file, data.reasoning, data.context_used);
        ...
        // Display results
        function displayResults(scenarios, report, csvFile, reasoning, contextUsed) {
            ...
            // NEW: Also display inline in Generate tab with reasoning
            const inlineContainer = document.getElementById('inlineResults');
            if (inlineContainer && reasoning) {
                inlineContainer.innerHTML = generateInlineResultsHTML(scenarios, report, reasoning, contextUsed);
        ...
        // Generate inline results HTML with tabs for scenarios, reasoning, and charts
        function generateInlineResultsHTML(scenarios, report, reasoning, contextUsed) {
            ...
            function simpleMarkdown(text) {
                ...
                // Headers
                result = result.replace(/### (.*?)$/gm, '<h4 style="color: var(--primary); margin-top: 1.5rem; margin-bottom: 0.5rem;">$1</h4>');
                result = result.replace(/## (.*?)$/gm, '<h3 style="color: var(--primary); margin-top: 2rem; margin-bottom: 1rem; border-bottom: 2px solid var(--border); padding-bottom: 0.5rem;">$1</h3>');
                result = result.replace(/# (.*?)$/gm, '<h2 style="color: var(--primary); margin-top: 2rem; margin-bottom: 1rem;">$1</h2>');
                ...
                // Code
                result = result.replace(/`(.*?)`/g, '<code style="background: var(--card-dark); padding: 2px 6px; border-radius: 3px; font-size: 0.875rem;">$1</code>');
                
                // Lists
                result = result.replace(/^- (.*?)$/gm, '<li style="margin-left: 1.5rem;">$1</li>');
            ...
            const contextBadges = contextUsed ? `
                <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; margin-top: 1rem;">
                    <span class="context-badge">📊 ${contextUsed.risk_factors_count} Risk Factors</span>
                    <span class="context-badge">🏢 ${contextUsed.counterparties_count} Counterparties</span>
                    <span class="context-badge">📄 ${contextUsed.contracts_count} Contracts</span>
                    ${contextUsed.knowledge_provided ? '<span class="context-badge">📚 Knowledge Used</span>' : ''}
                    <span class="context-badge">🤖 ${contextUsed.prompt_name}</span>
                </div>
            ` : '';
            ...
            return `
                <style>
                    ...
                    .reasoning-content h2, .reasoning-content h3, .reasoning-content h4 {
                        color: var(--primary);
                    }
                ...
                <div style="background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 1.5rem; margin-bottom: 2rem;">
                    <div style="display: flex; align-items: center; justify-content: between; margin-bottom: 1rem;">
                        <div>
                            <h3 style="margin: 0; color: var(--success);">✓ Scenarios Generated Successfully</h3>
                            <p style="color: var(--muted); font-size: 0.875rem; margin: 0.5rem 0 0 0;">
                                ${scenarios.length} scenarios with ${report.totalShocks} total shocks
                            </p>
                            ${contextBadges}
                        </div>
                    </div>
                    ...
                    <div class="result-tabs">
                        <button class="result-tab active" onclick="switchResultTab('scenarios-tab')">
                            📊 Scenarios (${scenarios.length})
                        </button>
                        <button class="result-tab" onclick="switchResultTab('reasoning-tab')">
                            🤖 AI Reasoning
                        </button>
                        <button class="result-tab" onclick="switchResultTab('charts-tab')">
                            📈 Visual Analysis
                        </button>
                    </div>
                    
                    <!-- Scenarios Tab -->
                    <div id="scenarios-tab" class="result-tab-content active">
                        <div class="scenarios-grid">
                            ${scenarios.map((s, i) => `
                                <div class="scenario-card">
                                    <div class="scenario-header">
                                        <div class="scenario-badge ${s.type?.toLowerCase()}">${s.type || 'Stress'}</div>
                                        <h4 style="margin: 0.5rem 0; color: var(--foreground);">${s.name}</h4>
                                        <p style="color: var(--muted); font-size: 0.875rem; margin: 0.5rem 0;">${s.description}</p>
                                    </div>
                                    <div class="scenario-stats">
                                        <div class="stat-item">
                                            <span class="stat-label">Shocks</span>
                                            <span class="stat-value">${s.num_shocks}</span>
                                        </div>
                                        <div class="stat-item">
                                            <span class="stat-label">NII Impact</span>
                                            <span class="stat-value ${s.impact.nii < 0 ? 'negative' : 'positive'}">${s.impact.nii?.toFixed(1) || 'N/A'}%</span>
                                        </div>
                                        <div class="stat-item">
                                            <span class="stat-label">EVE Impact</span>
                                            <span class="stat-value ${s.impact.eve < 0 ? 'negative' : 'positive'}">${s.impact.eve?.toFixed(1) || 'N/A'}%</span>
                                        </div>
                                    </div>
                                    <div style="margin-top: 1rem;">
                                        <button class="btn btn-outline btn-sm" onclick="toggleShocks(${i})" style="width: 100%;">
                                            View ${s.num_shocks} Shocks ▼
                                        </button>
                                        <div id="inline-shocks-${i}" style="display: none; margin-top: 1rem; max-height: 300px; overflow-y: auto;">
                                            ${s.shocks.slice(0, 15).map(shock => `
                                                <div style="padding: 0.5rem; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between;">
                                                    <span style="font-size: 0.875rem;">${shock.factor_id}</span>
                                                    <span style="font-size: 0.875rem; color: ${shock.value > 0 ? 'var(--success)' : 'var(--danger)'};">
                                                        ${shock.value > 0 ? '+' : ''}${shock.value}
                                                    </span>
                                                </div>
                                            `).join('')}
                                            ${s.num_shocks > 15 ? `<div style="padding: 0.5rem; text-align: center; color: var(--muted); font-size: 0.875rem;">... and ${s.num_shocks - 15} more</div>` : ''}
                                        </div>
                                    </div>
                                </div>
                            `).join('')}
                        ...
                        <div style="margin-top: 1.5rem; padding: 1rem; background: var(--secondary); border-radius: 8px;">
                            <p style="margin: 0; font-size: 0.875rem; color: var(--muted);">
                                💡 <strong>Tip:</strong> Full details and charts available in the <strong>Reporting</strong> tab
                            </p>
                        </div>
                    </div>
                    
                    <!-- Reasoning Tab -->
                    <div id="reasoning-tab" class="result-tab-content">
                        <div class="reasoning-content">
                            ${reasoning ? simpleMarkdown(reasoning) : '<p style="color: var(--muted);">Reasoning not available</p>'}
                        </div>
                        ${contextUsed ? `
                            <div style="margin-top: 2rem; padding: 1.5rem; background: var(--secondary); border-radius: 8px;">
                                <h4 style="margin: 0 0 1rem 0; color: var(--primary);">Context Summary</h4>
                                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; font-size: 0.875rem;">
                                    <div>
                                        <div style="color: var(--muted);">Risk Factors</div>
                                        <div style="font-weight: 600;">${contextUsed.risk_factors_count}</div>
                                        <div style="font-size: 0.75rem; color: var(--muted);">${contextUsed.risk_factors_types?.join(', ') || 'Various types'}</div>
                                    </div>
                                    <div>
                                        <div style="color: var(--muted);">Counterparties</div>
                                        <div style="font-weight: 600;">${contextUsed.counterparties_count}</div>
                                    </div>
                                    <div>
                                        <div style="color: var(--muted);">Contracts</div>
                                        <div style="font-weight: 600;">${contextUsed.contracts_count}</div>
                                    </div>
                                    <div>
                                        <div style="color: var(--muted);">Knowledge Docs</div>
                                        <div style="font-weight: 600;">${contextUsed.knowledge_provided ? 'Yes' : 'None'}</div>
                                        ${contextUsed.knowledge_size ? `<div style="font-size: 0.75rem; color: var(--muted);">${(contextUsed.knowledge_size / 1000).toFixed(1)}KB</div>` : ''}
                                    </div>
                                    <div>
                                        <div style="color: var(--muted);">Expert Prompt</div>
                                        <div style="font-weight: 600;">${contextUsed.prompt_name}</div>
                                        <div style="font-size: 0.75rem; color: var(--muted);">${contextUsed.prompt_description || ''}</div>
                                    </div>
                                    <div>
                                        <div style="color: var(--muted);">Scenarios Generated</div>
                                        <div style="font-weight: 600;">${contextUsed.num_scenarios_generated}</div>
                                        <div style="font-size: 0.75rem; color: var(--muted);">${contextUsed.total_shocks} total shocks</div>
                                    </div>
                                </div>
                            </div>
                        ` : ''}
                    ...
                    <div id="charts-tab" class="result-tab-content">
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 1.5rem;">
                            <div>
                                <h4 style="margin-bottom: 1rem;">Scenario Impact</h4>
                                <canvas id="inlineImpactChart"></canvas>
                            </div>
                            <div>
                                <h4 style="margin-bottom: 1rem;">Risk Factor Distribution</h4>
                                <canvas id="inlineDistributionChart"></canvas>
                            </div>
                        </div>
                        <div style="margin-top: 1.5rem; padding: 1rem; background: var(--secondary); border-radius: 8px;">
                            <p style="margin: 0; font-size: 0.875rem; color: var(--muted);">
                                📊 Interactive charts with full details available in the <strong>Reporting</strong> tab
                            </p>
                        </div>
                    </div>
                </div>
        ...
        function switchResultTab(tabId) {
            ...
            document.querySelector(`[onclick="switchResultTab('${tabId}')"]`)?.classList.add('active');
            document.getElementById(tabId)?.classList.add('active');
            
            // Initialize charts if switching to charts tab
            if (tabId === 'charts-tab' && currentScenarios && currentReport) {
                setTimeout(() => {
                    initInlineCharts(currentScenarios, currentReport);
                }, 100);
            }
        }
        
        // Toggle shock details
        function toggleShocks(index) {
            const shocksDiv = document.getElementById(`inline-shocks-${index}`);
            if (shocksDiv) {
                shocksDiv.style.display = shocksDiv.style.display === 'none' ? 'block' : 'none';
            }
        }
        
        // Initialize inline charts
        function initInlineCharts(scenarios, report) {
            // Similar to existing chart initialization but for inline display
            // Impact chart
            const impactCanvas = document.getElementById('inlineImpactChart');
            if (impactCanvas && typeof Chart !== 'undefined') {
                const impactCtx = impactCanvas.getContext('2d');
                new Chart(impactCtx, {
                    type: 'bar',
                    data: {
                        labels: scenarios.map(s => s.name.substring(0, 20)),
                        datasets: [
                            {
                                label: 'NII Impact (%)',
                                data: scenarios.map(s => s.impact.nii),
                                backgroundColor: 'rgba(59, 130, 246, 0.7)'
                            },
                            {
                                label: 'EVE Impact (%)',
                                data: scenarios.map(s => s.impact.eve),
            ...
            // Distribution chart
            const distCanvas = document.getElementById('inlineDistributionChart');
            if (distCanvas && typeof Chart !== 'undefined' && report.riskFactorDistribution) {
                const distCtx = distCanvas.getContext('2d');
                new Chart(distCtx, {
                    type: 'doughnut',
                    data: {
                        labels: report.riskFactorDistribution.map(d => d.name),
                        datasets: [{
                            data: report.riskFactorDistribution.map(d => d.count),
                            backgroundColor: [
                                'rgba(59, 130, 246, 0.7)',
                                'rgba(139, 92, 246, 0.7)',
                                'rgba(236, 72, 153, 0.7)',
        ...
        // Update analysis data status
        function updateAnalysisDataStatus() {
            const statusEl = document.getElementById('analysisDataStatus');
            if (!statusEl) return;
            
            if (dataCache.loaded) {
                const rfCount = dataCache.risk_factors?.length || 0;
                const cpCount = dataCache.counterparties?.length || 0;
                const ctCount = dataCache.contracts?.length || 0;
                statusEl.innerHTML = `📊 Data: ${rfCount} RF, ${cpCount} CP, ${ctCount} CT`;
                statusEl.style.color = 'var(--success)';
            } else {
                statusEl.innerHTML = 'Data: Not loaded';
                statusEl.style.color = 'var(--danger)';
            }
        }
        
        // Load prompts into analysis dropdown
        function loadAnalysisPrompts() {
            const select = document.getElementById('analysisPromptSelect');
            if (!select) return;
            
            // Copy from main prompt select
            const mainSelect = document.getElementById('promptSelect');
            if (mainSelect && mainSelect.options.length > 0) {
                select.innerHTML = mainSelect.innerHTML;
                // Select the same prompt as main if available
                if (mainSelect.value) {
        ...
        function addChatMessage(role, content, isMarkdown = false) {
            ...
            messageDiv.style.cssText = `
                ...
                ${role === 'user' ? 'margin-left: 2rem;' : 'margin-right: 2rem;'}
            `;
            
            const header = document.createElement('div');
            header.style.cssText = 'font-weight: 600; margin-bottom: 0.5rem; font-size: 0.875rem;';
            header.style.color = role === 'user' ? 'white' : 'var(--primary)';
            header.innerHTML = role === 'user' ? '👤 You' : '🤖 AI Assistant';
            
            const contentDiv = document.createElement('div');
            contentDiv.style.cssText = 'white-space: pre-wrap; line-height: 1.6;';
            contentDiv.style.color = role === 'user' ? 'white' : 'var(--foreground)';
            
            if (isMarkdown) {
                // Markdown rendering disabled to avoid JS parsing issues in Python templates.
        ...
        async function sendAnalysisMessage() {
            ...
            const sendBtn = document.getElementById('sendAnalysisBtn');
            const btnText = document.getElementById('sendBtnText');
            const btnSpinner = document.getElementById('sendBtnSpinner');
            const promptSelect = document.getElementById('analysisPromptSelect');
            ...
            // Check if data is loaded
            if (!dataCache.loaded) {
                alert('⚠ No data loaded. Please go to Input Data tab and load data first.');
                return;
            }
            ...
            if (!promptId) {
                alert('⚠ No prompt selected. Please select an LLM prompt.');
                return;
            }
            ...
            try {
                ...
                if (data.success) {
                    ...
                    addChatMessage('assistant', `Error: ${data.error}`, false);
                }
            } catch (error) {
        ...
        // Initialize analysis tab when switching to it
        function initAnalysisTab() {
            updateAnalysisDataStatus();
            loadAnalysisPrompts();
        }
        
        // Override switchTab to initialize analysis
...
def load_data_api():
    ...
    try:
        ...
        # Initialize enriched_data to None (will be populated for XML loading)
        enriched_data = None
...
def analysis_query():
    """
    AI Analysis endpoint - flexible LLM interaction for data analysis
    
    This endpoint allows users to ask ANY question about their data,
    not just scenario generation. The LLM acts as an analytical assistant.
    """
    try:
        data = request.json
        ...
        if not prompt_id:
            return jsonify({'success': False, 'error': 'No prompt selected'}), 400
        ...
        if knowledge_context:
            context_parts.append(f"Additional Knowledge:\n{knowledge_context}")
            print(f"📚 Added knowledge context ({len(knowledge_context)} characters)")
        
        # Add data summary
        data_summary = f"""
Current Data Summary:
- Risk Factors: {len(risk_factors)} loaded
  Types: {', '.join(set(rf.factor_type.value for rf in risk_factors))}
  
- Counterparties: {len(counterparties)} loaded
  Sample: {', '.join([cp.name for cp in counterparties[:5]])}
  {f'... and {len(counterparties) - 5} more' if len(counterparties) > 5 else ''}
  
- Contracts: {len(contracts)} loaded
  Types: {', '.join(set(c.contract_type.value for c in contracts))}
  Total value: Various currencies

You are an expert ALM analyst with access to this data. The user will ask you questions or request analysis.
Provide clear, actionable insights based on the loaded data.
"""
        ...
        print(f"📝 Analysis prompt built: {len(full_prompt)} characters")
        
        print("🤖 Sending to LLM...")
        
        # Call Ollama API directly for analysis
        try:
            import requests
            ...
            return jsonify({
                'success': False,
                'error': 'LLM request timed out. The query might be too complex.'
            }), 504
        
        except requests.exceptions.ConnectionError:
            return jsonify({
                'success': False,
                'error': 'Cannot connect to LLM server. Is Ollama running on localhost:11434?'
            }), 503
...
def generate():
    try:
        ...
        # ============================================================
        # GENERATE AI REASONING AND EXPLANATION
        # ============================================================
        print("\n🤖 Generating scenario reasoning and explanation...")
        
        reasoning_prompt = f"""You just generated {len(scenarios)} ALM stress scenarios. Please provide a comprehensive explanation of your reasoning.
...
"""
        for i, s in enumerate(scenarios, 1):
            reasoning_prompt += f"\n{i}. **{s.name}** ({s.scenario_type})\n"
            reasoning_prompt += f"   Description: {s.description}\n"
            reasoning_prompt += f"   Key shocks ({len(s.shocks)} total):\n"
            for shock in s.shocks[:5]:  # Show first 5 shocks per scenario
                reasoning_prompt += f"   • {shock.factor_id}: {shock.shock_type} = {shock.value}\n"
            if len(s.shocks) > 5:
                reasoning_prompt += f"   ... and {len(s.shocks) - 5} more shocks\n"
        
        reasoning_prompt += f"""
**DATA CONTEXT YOU HAD ACCESS TO:**
- Risk Factors: {len(risk_factors)} loaded
  Types: {', '.join(set(rf.factor_type.value for rf in risk_factors[:10]))}
  Sample: {', '.join([rf.factor_id for rf in risk_factors[:5]])}
  
- Counterparties: {len(counterparties)} loaded
  Sample: {', '.join([cp.name for cp in counterparties[:5]])}
  
- Contracts: {len(contracts)} loaded
  Types: {', '.join(set(c.contract_type.value for c in contracts[:10]))}

**KNOWLEDGE SOURCES:**
{f'✓ Knowledge documents were provided ({len(knowledge_context)} characters of domain knowledge)' if knowledge_context else '✗ No additional knowledge documents provided'}

**EXPERT GUIDANCE APPLIED:**
Prompt: {selected_prompt['name']}
Description: {selected_prompt.get('description', 'General ALM expertise')}

---

Please provide a **clear, structured explanation** covering these aspects:

## 1. Interpretation
How did you understand and interpret the user's request? What were the key requirements?

## 2. Data Considered  
Which specific risk factors and portfolio data influenced your scenario design? Be specific about the data you analyzed.

## 3. Risk Drivers Identified
What are the key risk drivers and relationships you considered? How do they interact in these scenarios?

## 4. Knowledge Sources Used
{f'How did the uploaded knowledge documents guide your reasoning? What insights did they provide?' if knowledge_context else 'No additional knowledge was provided, so you relied on your base expertise.'}

## 5. Expert Guidance Applied
How did the "{selected_prompt['name']}" prompt shape your approach? What specific expertise did it provide?

## 6. Reasoning Steps
Walk through your thought process step-by-step:
- How you translated the request into specific shocks
- Why you chose these particular risk factor combinations  
- How you determined shock magnitudes
- How scenarios relate to each other

## 7. Scenario Rationale
Why are these specific scenarios relevant and appropriate for this portfolio? What risks do they expose?

**Be specific and reference actual data from the loaded portfolio. Use markdown formatting for clarity.**
"""
        
        try:
            import requests
            
            print("  Calling LLM for reasoning generation...")
            reasoning_response = requests.post(
                ...
                json={
                    ...
                    'stream': False,
                    'options': {
                        'num_predict': 4000,  # Long response for detailed explanation
                        'temperature': 0.7
                    }
                },
                timeout=240  # 4 minutes for comprehensive reasoning
            )
            
            if reasoning_response.status_code == 200:
                reasoning_data = reasoning_response.json()
                reasoning_text = reasoning_data.get('response', '')
                print(f"✓ Generated reasoning: {len(reasoning_text)} characters")
            else:
                reasoning_text = f"**Reasoning generation encountered an API error (status {reasoning_response.status_code})**\n\nScenarios were generated successfully, but the explanation could not be produced."
                print(f"✗ Reasoning API error: {reasoning_response.status_code}")
        
        except requests.exceptions.Timeout:
            reasoning_text = "**Reasoning generation timed out**\n\nThe scenario generation was successful, but the explanation took too long to generate. Try requesting a simpler explanation or reduce the number of scenarios."
            print("✗ Reasoning timeout")
        
        except Exception as e:
            reasoning_text = f"**Reasoning generation failed**\n\nScenarios were created successfully, but explanation generation encountered an error: {str(e)}"
        ...
        return jsonify({
            ...
            'report': report,
            'csv_file': csv_file,
            'reasoning': reasoning_text,  # NEW: AI explanation
            'context_used': {  # NEW: Context summary for display
                'risk_factors_count': len(risk_factors),
                'risk_factors_types': list(set(rf.factor_type.value for rf in risk_factors)),
                'counterparties_count': len(counterparties),
                'contracts_count': len(contracts),
                'knowledge_provided': len(knowledge_context) > 0,
                'knowledge_size': len(knowledge_context),
                'prompt_name': selected_prompt['name'],
                'prompt_description': selected_prompt.get('description', ''),
                'num_scenarios_generated': len(scenarios),
                'total_shocks': sum(len(s.shocks) for s in scenarios)
            }
        })
