...
Counterparties: ${data.counterparties_count.toLocaleString()}`;
        ...
        function generateInlineResultsHTML(scenarios, report, reasoning, contextUsed) {
            ...
            function simpleMarkdown(text) {
                if (!text) return '';
                return text
                    .replace(/### (.*?)$/gm, '<h4 style="color: var(--primary); margin-top: 1.5rem; margin-bottom: 0.5rem;">$1</h4>')
                    .replace(/## (.*?)$/gm, '<h3 style="color: var(--primary); margin-top: 2rem; margin-bottom: 1rem; border-bottom: 2px solid var(--border); padding-bottom: 0.5rem;">$1</h3>')
                    .replace(/# (.*?)$/gm, '<h2 style="color: var(--primary); margin-top: 2rem; margin-bottom: 1rem;">$1</h2>')
                    .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
                    .replace(/\*(.*?)\*/g, '<em>$1</em>')
                    .replace(/`(.*?)`/g, '<code style="background: var(--card-dark); padding: 2px 6px; border-radius: 3px; font-size: 0.875rem;">$1</code>')
