...
def get_available_model_ids() -> List[Dict[str, Any]]:
    ...
    try:
        from load_alm_data import get_database_connection
        ...
        FROM (
            SELECT MODEL_ID, CONTRACT_ID, NULL as COUNTERPARTY_ID FROM dbo.CONTRACT WHERE MODEL_ID IS NOT NULL
            UNION ALL
            SELECT MODEL_ID, NULL, COUNTERPARTY_ID FROM dbo.COUNTERPARTY WHERE MODEL_ID IS NOT NULL
        ) combined
        GROUP BY MODEL_ID
        ORDER BY MODEL_ID
        """
...
def load_data_with_config(model_id: Optional[str] = None, limit: Optional[int] = 1000) -> Dict[str, Any]:
    """Load data with specified model_id and limit"""
    try:
        print(f"\n{'='*60}")
        print(f"Loading ALM data:")
        print(f"  Model ID: {model_id or 'ALL'}")
        print(f"  Limit: {limit or 'NONE'}")
        print(f"{'='*60}\n")
        
        # Load data from RiskPro
        risk_factors, counterparties, contracts = load_from_riskpro(limit_contracts=limit)
        
        # Filter by model_id if specified
        if model_id:
            filtered_counterparties = [
                cp for cp in counterparties 
                if hasattr(cp, 'extended_attributes') and 
                   cp.extended_attributes.get('model_id') == model_id
            ]
            
            filtered_contracts = [
                ct for ct in contracts
                if hasattr(ct, 'extended_attributes') and
                   ct.extended_attributes.get('model_id') == model_id
            ]
            
            counterparties = filtered_counterparties
...
def get_data_preview(data_type: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    if data_type == 'risk_factors' and cache['risk_factors']:
        for rf in cache['risk_factors'][:limit]:
            preview.append({
                'factor_id': rf.factor_id,
                'factor_type': rf.factor_type,
                'value': getattr(rf, 'current_value', 'N/A')
            })
    
    elif data_type == 'counterparties' and cache['counterparties']:
        for cp in cache['counterparties'][:limit]:
            preview.append({
                ...
                'pd': f"{cp.pd:.4f}",
                'sector': cp.sector,
                'model_id': cp.extended_attributes.get('model_id') if hasattr(cp, 'extended_attributes') else None
            })
    
    elif data_type == 'contracts' and cache['contracts']:
        for ct in cache['contracts'][:limit]:
            preview.append({
                'contract_id': ct.contract_id,
                'type': str(ct.contract_type),
                'notional': f"{ct.notional:,.2f}",
                'maturity': str(ct.maturity_date),
                'product_type': ct.extended_attributes.get('product_type') if hasattr(ct, 'extended_attributes') else None,
                'model_id': ct.extended_attributes.get('model_id') if hasattr(ct, 'extended_attributes') else None
            })
...
def load_data():
    """Legacy function - loads data if not already loaded"""
    if not cache['loaded']:
        print("Loading ALM data with defaults...")
        result = load_data_with_config(model_id=None, limit=1000)
                ...
                <div class="card">
                    ...
                    <div style="background: rgba(20, 184, 166, 0.05); border: 1px solid rgba(20, 184, 166, 0.2); border-radius: 10px; padding: 1.5rem; margin-bottom: 1.5rem;">
                        ...
                        <div class="form-group">
                            <label for="modelIdSelect">Model ID *</label>
                            <select id="modelIdSelect" style="width: 100%;">
                                <option value="">Loading models...</option>
                            </select>
                            <p style="font-size: 0.75rem; color: var(--muted); margin-top: 0.5rem;" id="modelInfo">
                                Select a model to load contracts and counterparties
                            </p>
                        </div>
                        
                        <div class="form-group">
                            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; margin-bottom: 0.5rem;">
                                <input type="checkbox" id="applyLimitCheck" checked onchange="toggleLimitInput()">
                        ...
                        <button class="btn btn-primary btn-full" onclick="loadDataNow()" id="loadDataBtn">
                            🔄 Load Data from RiskPro
                        </button>
                    </div>
                ...
                <div class="status-panel">
                    <div class="status-card">
                        ...
                        <div class="status-row">
                            ...
                            <span id="lastUpdated" class="status-value" style="font-size: 0.75rem;">Never</span>
                        </div>
                    </div>
                    ...
                    <div class="status-card">
                        ...
                        <div style="font-size: 0.875rem; color: var(--muted); line-height: 1.6;">
                            <p style="margin-bottom: 0.75rem;"><strong style="color: var(--foreground);">Step 1:</strong> Select a Model ID from dropdown</p>
                            <p style="margin-bottom: 0.75rem;"><strong style="color: var(--foreground);">Step 2:</strong> Configure record limit (optional)</p>
                            <p style="margin-bottom: 0.75rem;"><strong style="color: var(--foreground);">Step 3:</strong> Click "Load Data from RiskPro"</p>
...
Generate 3 stress scenarios for interest rate risk, credit spreads, and FX rates with severe but plausible shocks based on historical crisis events."
        ...
        <div class="tab-content" id="prompts-content">
            <div class="card">
                <div class="card-header">
                    <h2 class="card-title">Prompt Templates</h2>
...
<!-- Results Tab -->
    ...
    <script>
        ...
        async function loadAvailableModels() {
            ...
            if (!select) {
                console.error('❌ ERROR: modelIdSelect element not found in DOM!');
                return;
            }
            
            try {
                console.log('Fetching from /api/data/models...');
                ...
                if (data.success && data.models && data.models.length > 0) {
                    ...
                    console.log(`✅ SUCCESS: Loaded ${data.models.length} models into dropdown`);
                    console.log('Model IDs:', data.models.map(m => m.model_id));
                    
                    // Update info text
                    const modelInfo = document.getElementById('modelInfo');
                    if (modelInfo) {
                        modelInfo.textContent = `${data.models.length} models available - Select one to load data`;
                        modelInfo.style.color = 'var(--success)';
                    }
                    
                } else if (data.success && (!data.models || data.models.length === 0)) {
                    console.warn('⚠️ WARNING: API returned success but zero models');
                    select.innerHTML = '<option value="">No models found in database</option>';
                    alert('Warning: No model IDs found in RiskPro database. Check if CONTRACT table has MODEL_ID values.');
                ...
                console.error('❌ EXCEPTION: Failed to load models:', error);
                select.innerHTML = '<option value="">Error: Cannot connect</option>';
                alert('Failed to load models from database. Error: ' + error.message + '\n\nCheck browser console (F12) for details.');
            }
        }
        
        function toggleLimitInput() {
            const checked = document.getElementById('applyLimitCheck').checked;
            ...
            if (checked) {
                ...
                limitGroup.style.opacity = '0.5';
                                ...
                                Counterparties: ${data.counterparties_count}
                        Contracts: ${data.contracts_count}`, 'success');
                    
                    updateDataStatus();
                    updatePreview();
                } else {
                    showDataMessage('Error: ' + data.error, 'error');
                }
            } catch (error) {
        ...
        async function updateDataStatus() {
            try {
                ...
                if (data.success) {
                    document.getElementById('dataLoadedStatus').textContent = data.loaded ? 'Yes' : 'No';
                    document.getElementById('dataLoadedStatus').style.color = data.loaded ? 'var(--success)' : 'var(--muted)';
                    
                    document.getElementById('currentModelId').textContent = data.model_id || 'All';
                    document.getElementById('riskFactorsCount').textContent = data.counts.risk_factors;
                    document.getElementById('counterpartiesCount').textContent = data.counts.counterparties;
                    document.getElementById('contractsCount').textContent = data.counts.contracts;
        ...
        function showDataMessage(text, type) {
            ...
            const formattedText = lines.map(line => `<div>${line}</div>`).join('');
            
            container.innerHTML = `<div style="background: ${bgColor}; border: 1px solid ${borderColor}; padding: 1rem; border-radius: 8px; margin-bottom: 1rem; color: ${textColor};">${formattedText}</div>`;
            setTimeout(() => { container.innerHTML = ''; }, 5000);
        }
        
        // Initialize on page load
        document.addEventListener('DOMContentLoaded', async function() {
            console.log('🚀 Initializing ALM Scenario Generator...');
            await loadAvailableModels();  // Load model dropdown
            await loadPrompts();           // Load saved prompts
            await updateDataStatus();      // Update data status panel
        ...
        // Load data before generating scenarios
        async function loadDataForGeneration() {
            ...
            // Validate inputs
            if (!modelId || modelId === '') {
                showError('Please select a Model ID');
                return false;
            }
            
            if (applyLimit && (!limit || limit <= 0)) {
                showError('Please enter a valid limit (positive number)');
                return false;
            }
            
            console.log('📊 Loading data with config:', {modelId, applyLimit, limit});
            ...
            try {
                ...
                if (data.success) {
                    const message = `✓ Data Loaded Successfully
                        ...
                        Limit: ${data.limit ? data.limit + ' records' : 'No limit'}
                        Contracts: ${data.contracts_count}
                        Counterparties: ${data.counterparties_count}`;
                    
                    showDataLoadSuccess(message);
                    console.log('✓ Data loaded:', data);
                    return true;
                } else {
                    showError('Data loading failed: ' + data.error);
                    return false;
                }
            } catch (error) {
        ...
        function showDataLoadSuccess(message) {
            const lines = message.split('\n').map(l => l.trim()).filter(l => l);
            const formatted = lines.map(line => `<div>${line}</div>`).join('');
            
            const container = document.getElementById('inlineResults');
            container.innerHTML = `<div style="background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.2); padding: 1rem; border-radius: 8px; margin-bottom: 1rem; color: var(--success);">${formatted}</div>`;
            container.classList.remove('hidden');
        }
        
        
        // Load data from RiskPro (from Data Loading tab)
        async function loadDataNow() {
            ...
            // Validate
            if (!modelId || modelId === '' || modelId === 'Loading models...') {
                showDataMessage('Please select a valid Model ID', 'error');
                return;
            }
            ...
            console.log('📊 Loading data:', {modelId, applyLimit, limit});
            ...
            try {
                ...
                if (data.success) {
                    ...
                    showDataMessage('Data loading failed: ' + data.error, 'error');
                ...
                btn.disabled = false;
                btn.innerHTML = '🔄 Load Data from RiskPro';
            }
        }
        ...
        // Handle form submission - LOAD DATA FIRST, then generate
...
@app.route('/api/data/models', methods=['GET'])
def get_models():
...
def load_data_api():
    ...
    try:
        ...
        limit = int(data.get('limit', 1000)) if apply_limit else None
        
        # Convert empty string to None
        if model_id == '' or model_id == 'all':
            model_id = None
        
        result = load_data_with_config(model_id=model_id, limit=limit)
...
@app.route('/api/data/preview/<data_type>', methods=['GET'])
def get_preview(data_type):
    """Get data preview"""
    try:
        limit = int(request.args.get('limit', 5))
        preview = get_data_preview(data_type, limit=limit)
        
        return jsonify({
            'success': True,
            'data_type': data_type,
            'preview': preview,
            'total_count': len(cache.get(data_type, []))
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_data_status():
    ...
    try:
        return jsonify({
            ...
            'timestamp': cache.get('load_timestamp'),
            'counts': {
                'risk_factors': len(cache['risk_factors']) if cache['risk_factors'] else 0,
                'counterparties': len(cache['counterparties']) if cache['counterparties'] else 0,
                'contracts': len(cache['contracts']) if cache['contracts'] else 0
            }
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
