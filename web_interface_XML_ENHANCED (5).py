...
<head>
    ...
    <title>ALM & Risk using Gen AI | Risk HUB Platform</title>
    ...
    <header class="header">
        <div class="header-content">
            <div class="logo">
                ...
                <div class="logo-text">
                    <h1>ALM & Risk using Gen AI</h1>
                    <p>Risk HUB Platform</p>
                </div>
            </div>
                        ...
                        <div style="display: flex; gap: 1rem;">
                            <label class="radio-card" style="flex: 1;">
                                ...
                                <div class="radio-card-content">
                                    ...
                                    <span class="radio-card-title">RiskPro Database</span>
                                    <span class="radio-card-desc">Connect to OneSumX/RiskPro DB</span>
                                </div>
                            </label>
                            <label class="radio-card" style="flex: 1;">
                                ...
                                <div class="radio-card-content">
                                    ...
                                    <span class="radio-card-desc">Upload OneSumX XML exports</span>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div id="dbParameters" class="source-params">
                        <div style="background: rgba(20, 184, 166, 0.05); border: 1px solid rgba(20, 184, 166, 0.2); border-radius: 10px; padding: 1.5rem;">
                            <div style="display: flex; align-items: center; gap: 8px; color: var(--primary); font-weight: 600; margin-bottom: 1rem; font-size: 1rem;">
                                🏦 RiskPro Database Configuration
                            </div>
        ...
        async function fetchDiscoveredAttributes() {
            try {
                ...
                if (data.success && (data.model_parameters || data.counterparties || data.contracts || data.observations)) {
                    ...
                    // Display model parameter attributes (NEW!)
                    if (data.model_parameters) {
                        ...
                        const allModelParamAttrs = data.model_parameters.attributes.filter(a => a.coverage >= 50);
                        const showInitial = Math.min(8, allModelParamAttrs.length);
                        
                        // Render initial 8 attributes
                        allModelParamAttrs.slice(0, showInitial).forEach(attr => {
                            html += `
                        ...
                        if (allModelParamAttrs.length > 8) {
                            html += `<div id="modelParamAttrsExtra" style="display: none;">`;
                            
                            allModelParamAttrs.slice(8).forEach(attr => {
                                html += `
                                    <div style="margin-bottom: 0.5rem; padding: 0.5rem; background: var(--secondary); border-radius: 6px;">
                                        <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
                                            <span style="color: var(--foreground); font-weight: 500; font-size: 0.75rem;">${attr.name}</span>
                            ...
                            // Add toggle button
                            html += `
                                <button onclick="toggleAttributes('modelParamAttrs')" 
                                    ...
                                    <span id="modelParamAttrsToggleText">▼ Show ${allModelParamAttrs.length - 8} more attributes</span>
                                </button>
                            `;
                        }
...
def load_data_api():
    ...
    try:
        ...
        if source == 'db':
            ...
            risk_factors_source = 'RiskPro DB' if load_risk_factors_db else 'Sample Data'
