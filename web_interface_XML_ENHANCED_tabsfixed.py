...
<script>
// Fallback tab navigation (prevents UI lock-up if another inline script fails to parse)
(function () {
  ...
  window.switchTab = window.switchTab || function (tabId) {
    try {
      _qsa('.tab').forEach(btn => btn.classList.remove('active'));
      _qsa('.tab-content').forEach(p => p.classList.remove('active'));

      const btn = _qs('[data-tab="' + tabId + '"]');
      const panel = document.getElementById(tabId + '-content');
      if (!btn || !panel) {
        console.error('switchTab: missing tab or panel', { tabId, btn: !!btn, panel: !!panel });
        return;
      }
      btn.classList.add('active');
      panel.classList.add('active');
    } catch (e) {
      console.error('switchTab fallback error', e);
    }
  };

  window.handleDataSourceChange = window.handleDataSourceChange || function () {
    try {
      const selected = _qs('input[name="dataSource"]:checked');
      const src = selected ? selected.value : 'db';
      const db = document.getElementById('dbParameters');
      const xml = document.getElementById('xmlParameters');
      if (db) db.style.display = (src === 'db') ? 'block' : 'none';
      if (xml) xml.style.display = (src === 'xml') ? 'block' : 'none';
    } catch (e) {
      console.error('handleDataSourceChange fallback error', e);
    }
  };

  document.addEventListener('DOMContentLoaded', function () {
    // Ensure first active tab is displayed
    const activeBtn = _qs('.tab.active');
    if (activeBtn) {
    ...
    // Ensure data source sections are consistent on load
